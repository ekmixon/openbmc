...
def _run_gpiocli(
    gpio_cmd, chip=None, name=None, offset=None, shadow=None, gpio_args=None
):
...
def gpio_set_direction(name, direction, chip=_GPIO_CHIP_ASPEED):
    ...
    if _shadow_exists(name):
        ...
        _run_gpiocli("set-direction", gpio_args=direction, chip=chip, name=name)
...
def gpio_set_value(name, value, chip=_GPIO_CHIP_ASPEED, change_direction=True):
    ...
    if _shadow_exists(name):
        ...
        _run_gpiocli(cmd, gpio_args=str(value), chip=chip, name=name)
