...
def config_func(args):
    ...
    try:
        ...
        print(
            'The code is not smart enough to set function "%s": %s\n'
            "Please set the function manually." % (args.function, str(e))
        )
...
def main():
    ...
    ap.add_argument("--debug", action="store_true", help="Enable debug messages")

    subparser = ap.add_subparsers()

    dump_parser = subparser.add_parser("dump", help="Dump the current HW GPIO settings")
    dump_parser.set_defaults(func=dump_func)

    config_parser = subparser.add_parser(
        "config", help="Configure one HW pin to a function"
    )
    config_parser.add_argument("function", help="The function name to set")
    config_parser.set_defaults(func=config_func)

    export_parser = subparser.add_parser("export", help="Export a GPIO directory")
    export_parser.add_argument("gpio", help='The GPIO name, i.e. "A4", or "GPIOD2"')
    export_parser.add_argument(
        "shadow", default=None, help="The shadow name given to this GPIO"
    )
    export_parser.set_defaults(func=export_func)

    set_parser = subparser.add_parser("set", help="Set a value for a GPIO")
    set_parser.add_argument("gpio", help="The GPIO name or number")
    set_parser.add_argument("value", type=int, choices=[0, 1], help="The value to set")
    set_parser.add_argument(
        "-k", "--keep", action="store_true", help="Keep the GPIO direction"
    )
    set_parser.set_defaults(func=set_func)

    get_parser = subparser.add_parser("get", help="Get a GPIO's value")
    get_parser.add_argument("gpio", help="The GPIO name or number")
    get_parser.add_argument(
        "-k", "--keep", action="store_true", help="Keep the GPIO direction"
    )
    get_parser.set_defaults(func=get_func)

    info_parser = subparser.add_parser("info", help="Get a GPIO's info")
    info_parser.add_argument("gpio", help="The GPIO name or number")
